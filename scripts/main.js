...
function setupDiceRollHooks() {
  ...
  Hooks.on('preCreateChatMessage', async (message, data, options, userId) => {
    ...
    for (let roll of message.rolls) {
      ...
      modifiedRolls.push(roll);
    }
    
    // If rolls were modified, update the message with the new rolls
    if (modified) {
      // Update the rolls array
      message.updateSource({ rolls: modifiedRolls });
      
      // Also update the content if it exists to force re-render
      if (message.content) {
        // The content will be regenerated by Foundry with the updated rolls
        message.updateSource({ content: message.content });
      }
